...
async function fetchCommitActivity(owner: string, repo: string): Promise<Map<string, number>> {
  ...
  try {
    // Fetch recent commits (last 100) to get per-file commit heuristics
    const commits = await githubFetch<Array<{ sha: string; commit: { message: string } }>>(
      `${GITHUB_API}/repos/${owner}/${repo}/commits?per_page=100`,
    );

    // For a heuristic, we'll use commit count as a proxy
    // We can't fetch individual commit diffs for every commit without heavy API usage
    // So we use participation stats instead
    const stats = await githubFetch<GitHubCommitStats[]>(
      `${GITHUB_API}/repos/${owner}/${repo}/stats/contributors`,
    );

    // Total commits for the repo
    const totalCommits = stats.reduce((sum, contributor) => sum + contributor.total, 0);
    commitMap.set('__total__', totalCommits);
  } catch {
    // Non-critical: commit activity is optional enrichment
...
export async function fetchRepositoryData(owner: string, repo: string): Promise<RepoData> {
  ...
  const files: RepoFile[] = blobs.map((blob) => {
    ...
    // Heuristic commit count: distribute total commits proportionally by file size
