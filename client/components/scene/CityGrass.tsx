  ...
  const { patches, blades, benches } = useMemo(() => {
    ...
    for (let i = 0; i < districts.length; i++) {
      for (let j = i + 1; j < districts.length; j++) {
        ...
        if (dist < 15 && dist > 3 && isFree(mx, mz) && isInCity(mx, mz)) {
          ...
          if (rand() > 0.5) {
            benchPositions.push({ x: mx + (rand() - 0.5) * 1.5, z: mz + (rand() - 0.5) * 1.5, rot: rand() * Math.PI });
          }
        }
      }
    }
    ...
    districts.forEach((d) => {
      ...
      for (let t = -hw + 0.5; t < hw; t += GRASS_STRIP_SPACING + rand() * 2) {
        ...
        if (isFree(px, tz1) && isInCity(px, tz1)) {
        ...
        if (isFree(px, tz2) && isInCity(px, tz2)) {
          const sw = 0.3 + rand() * 0.6;
      ...
      for (let t = -hd + 0.5; t < hd; t += GRASS_STRIP_SPACING + rand() * 2) {
        ...
        if (isFree(tx1, pz) && isInCity(tx1, pz)) {
        ...
        if (isFree(tx2, pz) && isInCity(tx2, pz)) {
          const sd = 0.3 + rand() * 0.6;
    ...
    for (let gx = minX; gx <= maxX; gx += gridStep) {
      for (let gz = minZ; gz <= maxZ; gz += gridStep) {
        ...
        if (rand() > 0.65 && isFree(px, pz) && isInCity(px, pz)) {
          ...
          for (let b = 0; b < bladeCount; b++) {
            bladePositions.push({
              x: px + (rand() - 0.5) * sw * 1.8,
              z: pz + (rand() - 0.5) * sd * 1.8,
          ...
          // Occasional bench
          if (rand() > 0.8) {
            benchPositions.push({ x: px + sw * 0.6, z: pz, rot: rand() * Math.PI });
          }
        }
      }
    }
